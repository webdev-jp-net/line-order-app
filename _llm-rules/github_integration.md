...
## 運用手順
1. **必須**: IssueやPRを新規作成する際は、内容に応じたラベルを必ず付与する（タスクタイプ + 優先度 + 該当する技術スタック）
2. **必須**: descriptionの編集や、関連ブランチでの開発進行中に内容や状況が変化した場合も、ラベルやステータス（open/closed、進行中、レビュー待ち等）の変更をAIが能動的に提案・実行する。
3. **必須**: IssueやPRの管理状況が常に最新状態に保たれるよう維持する。
...
#### タスクタイプラベル
- **type:design**: 設計作業（システム設計、アーキテクチャ設計、API設計等）
- **type:implementation**: 実装作業（フロントエンド、バックエンド、インフラ実装）
- **type:verification**: 検証作業（テスト、レビュー、品質確認）
- **type:bug**: バグ修正（設計・実装・検証の各段階で発生する問題の修正）
- **type:docs**: ドキュメントの作成・更新
- **type:chore**: その他の作業（依存関係更新、設定変更など）

#### 優先度ラベル
- **priority:critical**: 即座に対応が必要（システム停止、重大なセキュリティ問題）
- **priority:high**: 早急な対応が必要（主要機能の不具合、重要な新機能）
- **priority:medium**: 通常の開発サイクルで対応
...
- **frontend**: UI実装
- **backend**: サーバーサイド実装
- **infrastructure**: クラウドインフラ実装
- **ステータス**: status:blocked（ブロック時のみ）
- **難易度**: difficulty:easy、difficulty:medium、difficulty:hard
- **依存関係**: dependency:design、dependency:implementation（依存がある場合のみ）
- **初心者向け**: good-first-issue

### Issue作成・管理の運用ルール
1. **Issue作成時**: 
   - 必須ラベル（type、priority）を必ず付与
   - type: タスクタイプ（design/implementation/verification/bug/docs/chore）
   - priority: 優先度（critical/high/medium/low）
   - stack: 技術スタック（該当する場合のみfrontend/backend/infrastructure）
2. **進行中**: 問題発生時にstatus:blockedラベルを追加
3. **完了時**: 完了後は通常Issueをclose（ラベル操作不要）
4. **依存関係**: 他タスクとの依存がある場合のみdependencyラベルを使用
...
- とくに箇条書きやセクション分け、ラベルやIssue番号の記載時は適切な改行・空行を挿入し視認性を高めること。
